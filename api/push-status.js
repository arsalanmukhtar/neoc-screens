// GET /api/push-status → { enabled, stations: { "GCOP": 1, … }, total }
// How many devices receive each station's alerts (counts only, no device details).
// Reads the running tally in push:counts (one command); builds it once from the station lists if missing.

import { redis, pushReady, STATION_IDS, COUNTS_KEY, syncCounts } from './_push.js';

export default async function handler(req, res) {
    res.setHeader('Cache-Control', 'no-store');
    if (!pushReady) return res.status(200).json({ enabled: false, stations: {}, total: 0 });
    try {
        if (!(await redis.exists('push:counts:ready'))) {
            await syncCounts(STATION_IDS);
            await redis.set('push:counts:ready', 1);
        }
        const raw = (await redis.hgetall(COUNTS_KEY)) || {};
        const stations = {};
        STATION_IDS.forEach(id => { const n = Number(raw[id]) || 0; if (n > 0) stations[id] = n; });
        const total = Object.values(stations).reduce((a, b) => a + b, 0);
        return res.status(200).json({ enabled: true, stations, total });
    } catch (err) {
        console.error('[push-status]', err);
        return res.status(500).json({ error: 'Storage error' });
    }
}

...
- **WhatsApp** (phone app only, **switched off for now** — `WHATSAPP_ON` in `js/app.js`): the third alert button sends a WhatsApp message to the station's number. The numbers live on the bridge VM (`whatsapp-bridge/`), never in this repo, and the alert ends with "Reply ACK to acknowledge" — a reply is recorded like any other acknowledgement. The desktop dashboard does not offer it.
